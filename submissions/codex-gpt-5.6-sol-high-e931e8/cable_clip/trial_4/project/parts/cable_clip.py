...
def cable_clip(bundle_diameter=measured("bundle_diameter"), draft=False):
    ...
    right_wall = Box(
        ...
        Pos(channel_width + wall_thickness, 0.0, base_thickness)
    )

    body = base + left_wall + right_wall

    tab_center_x = channel_outer_width + tab_length / 2.0
    screw_hole = Cylinder(
        screw_hole_width / 2.0,
        base_thickness,
        align=(Align.CENTER, Align.CENTER, Align.MIN),
    ).moved(Pos(tab_center_x, part_length / 2.0, 0.0))
