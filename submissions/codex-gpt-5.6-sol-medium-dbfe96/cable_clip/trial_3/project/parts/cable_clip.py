...
def cable_clip(bundle_diameter=measured("bundle_diameter")):
    ...
    left_wall = Box(
        ...
        channel_depth,
        align=(Align.MIN, Align.MIN, Align.MIN),
    ).moved(Pos(0, 0, base_thickness))
    right_wall = Box(
        wall_thickness,
        part_length,
        channel_depth,
        align=(Align.MIN, Align.MIN, Align.MIN),
    ).moved(Pos(channel_outer_width - wall_thickness, 0, base_thickness))
    ...
    tab = Box(
        ...
        base_thickness,
        align=(Align.MIN, Align.MIN, Align.MIN),
    ).moved(Pos(channel_outer_width, 0, 0))
    screw_hole = Cylinder(
        screw_hole_width / 2.0,
        base_thickness,
        align=(Align.CENTER, Align.CENTER, Align.MIN),
    ).moved(Pos(channel_outer_width + tab_length / 2.0, part_length / 2.0, 0))
