...
VERDICTS = {
    ("claude", "claude-fable-5", "low"): "Eighteen attempts, eighteen parts worth printing, every job included, and it recorded the unmeasured dimension as a guess every time. About four minutes and a dollar eighty a part, which is half the time and half the money of the same model at high effort, and high effort is the one that misses. This is one person's run of three attempts a job, so it is a thinner sample than the Grok row under it.",
