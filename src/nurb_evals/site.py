...
VERDICTS = {
    ...
    ('claude', 'claude-fable-5', 'max'): 'Eighteen attempts, eighteen parts worth printing, every job included, and it recorded the unmeasured dimension as a guess every time. It checks its own work hard, better than twice the measuring, cutting open and re-rendering that the same model does at medium effort. That buys nothing here, because medium sweeps the same eighteen jobs in a quarter of the time for a quarter of the money. At about thirteen minutes and five dollars eighty a part, this is the most expensive row on the board, so pay it only for a part you cannot re-run. Three attempts a job from one person, so a thinner sample than the Grok rows.',
